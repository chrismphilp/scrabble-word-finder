...
class GameTest extends AnyFunSuite {
  ...
  test("Should correctly find highest scoring word from single word") {
    ...
    rack.setRack(List(G(), A(), Z(), E()).to(ListBuffer))

    val game: Game = new Game(board, trie, rack)
    game.updateBoard()

    val highestScoringWord: (String, Int, Int, Int, Direction.Value) = game.findHighestScoringWord()

    assert(highestScoringWord._1 === "")
  }
}
